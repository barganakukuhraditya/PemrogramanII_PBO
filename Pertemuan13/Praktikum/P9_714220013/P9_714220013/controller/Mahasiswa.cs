...
namespace P9_714220013.controller
{
    class Mahasiswa
    {
        ...
        public bool Update(M_mahasiswa mahasiswa, string npm)
        {
            ...
            {
                koneksi.OpenConnection();
                koneksi.ExecuteQuery("UPDATE t_mahasiswa SET nama='" + mahasiswa.Nama + "'," + "angkatan='" + mahasiswa.Angkatan + "'," + "alamat='" + mahasiswa.Alamat + "'," + "email='" + mahasiswa.Email + "'," + "nohp='" + mahasiswa.Nohp + "' WHERE npm = '" + mahasiswa.Npm + "'");
                status = true;
